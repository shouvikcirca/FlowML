...
@click.option("--vocab-length-start", default=80, type=int)
@click.option("--vocab-length-end", default=100, type=int)
@click.option("--seq-padding-style", default="post", type=str)
#@click.option("--seq-truncating-style", default="post", type=str)
@click.option("--embedding-dim-start", default=50, type=int)
@click.option("--embedding-dim-end", default=50, type=int)
@click.option("--bs-start", default=64, type=int)
@click.option("--bs-end", default=64, type=int)
@click.option("--epochs-start", default=5, type=int)
@click.option("--epochs-end", default=5, type=int)
...
def getDataPath():
	return '/home/centos/Shouvik/mlflow_workshop/cyberbullying_classification/hpSearch/'


def getExperimentName():
	return 'RayExperiments'
...
def train(config):
	...
	signature = ''
	client = MlflowClient()
	exp_id = mlflow.get_experiment_by_name(getExperimentName()).experiment_id
	...
	with mlflow.start_run(run_id = run.info.run_id) as active_run:
		...
		filePath = getDataPath()#getDataPath()
